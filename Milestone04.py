...
def update_frame():
    ...
    if results.multi_hand_landmarks:
        ...
        for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            if total == 0:
                ...
                cv2.rectangle(frame, (10, 10), (220, 50), (0, 140, 255), -1)
                cv2.putText(frame, f"{gesture_state} Gesture",
                            (20, 40),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.7,
                            (255, 255, 255),
    ...
    volume_metric.config(text=f"Volume\n{volume_percent}%")
    distance_metric.config(text=f"Distance\n{distance_value}px")
    latency_metric.config(text=f"Response\n{latency} ms")
...
title = tk.Label(root, text="Hand Gesture Volume Control System",
                 font=("Arial", 22, "bold"),
                 bg="#1e1e1e",
                 fg="white",
                 pady=10)
...
tk.Button(btn_frame, text="Start Camera", bg="#2254f8", command=start_camera, fg="white",
          font=("Arial",12,"bold"), width=18, height=2).grid(row=0, column=0, padx=10)
tk.Button(btn_frame, text="Stop Camera", bg="#2254f8", command=stop_camera, fg="white",
          font=("Arial",12,"bold"), width=18, height=2).grid(row=0, column=1, padx=10)
tk.Button(btn_frame, text="Exit", bg="#2254f8", command=root.destroy, fg="white",
          font=("Arial",12,"bold"), width=18, height=2).grid(row=0, column=2, padx=10)
...
canvas = tk.Canvas(panel_container, bg="#1e1e1e", width=420, highlightthickness=0)
...
# ------------Perfomance Matrix---------------
tk.Label(panel, text="Performance Metrics", bg="#010779",
         fg="white", font=("Arial", 12, "bold")).pack(pady=(10,0))

metric_frame = tk.Frame(panel, bg="#1e1e1e")
metric_frame.pack(pady=5)

volume_metric = tk.Label(metric_frame, text="Volume\n0%",
                         bg="#2b2b2b", fg="orange", width=10)
volume_metric.grid(row=0, column=0, padx=5, pady=5)

distance_metric = tk.Label(metric_frame, text="Distance\n0px",
                           bg="#2b2b2b", fg="cyan", width=10)
distance_metric.grid(row=0, column=1, padx=5, pady=5)

latency_metric = tk.Label(metric_frame, text="Response\n0 ms",
                          bg="#2b2b2b", fg="yellow", width=10)
latency_metric.grid(row=1, column=0, padx=5, pady=5)

accuracy_metric = tk.Label(metric_frame, text="Accuracy\n98%",
                           bg="#2b2b2b", fg="lightgreen", width=10)
accuracy_metric.grid(row=1, column=1, padx=5, pady=5)
